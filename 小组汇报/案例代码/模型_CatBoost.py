...
# 模型训练
print("\n训练CatBoost模型...")
model = CatBoostRegressor(n_estimators=100, max_depth=6, learning_rate=0.1, random_state=42, verbose=0)
...
ax1.set_title('CatBoost - 实际vs预测', fontsize=14, fontweight='bold')
...
ax2.set_title('CatBoost - 残差分布', fontsize=14, fontweight='bold')
...
ax3.set_title('CatBoost - 特征重要性', fontsize=14, fontweight='bold')
ax3.set_xlabel('重要性（%）', fontsize=12)
ax3.set_ylabel('特征', fontsize=12)

plt.tight_layout()
plt.savefig('CatBoost_结果.png', dpi=150, bbox_inches='tight')
plt.close()

print(f"\n结果已保存: CatBoost_结果.png")
print("="*70)
